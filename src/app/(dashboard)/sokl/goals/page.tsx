...
export default function SoklGoalsPage() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    if (!user) return
    supabase.from('goals').select('*').eq('user_id', user.id).eq('business_id', 'sokl')
  ...
  const saveGoal = async () => {
    ...
    setSaving(true)
    const payload = { title: form.title, description: form.description || null, period: form.period as Goal['period'], target_date: form.target_date || null, progress: parseInt(form.progress) || 0, status: form.status as Goal['status'], business_id: 'sokl', user_id: user.id, category: 'business' }
    if (editGoal) {
  ...
  return (
    <div className="flex flex-col flex-1">
      <Header title="SOKL — Цели" subtitle="Бизнес-цели SOKL" actions={<Button size="sm" onClick={() => { setEditGoal(null); setForm({ title: '', description: '', period: 'monthly', target_date: '', progress: '0', status: 'active' }); setModalOpen(true) }}><Plus size={16} /> Цель</Button>} />
      <div className="p-6 space-y-3">
        {goals.map(goal => (
          <Card key={goal.id}><CardContent className="p-4">
            <div className="flex items-start gap-3">
              <Target size={18} className="text-cyan-500 mt-0.5 flex-shrink-0" />
              ...
              <div className="flex gap-1">
                <button onClick={() => { setEditGoal(goal); setForm({ title: goal.title, description: goal.description || '', period: goal.period, target_date: goal.target_date || '', progress: goal.progress.toString(), status: goal.status }); setModalOpen(true) }} className="p-1.5 rounded text-gray-400 hover:text-cyan-500"><Edit2 size={14} /></button>
        ...
        {!loading && goals.length === 0 && (
          <div className="text-center py-16"><p className="text-4xl mb-3">🎯</p><p className="text-gray-500">Нет целей для SOKL</p><Button className="mt-4" onClick={() => setModalOpen(true)}><Plus size={16} /> Добавить цель</Button></div>
        )}
      </div>
